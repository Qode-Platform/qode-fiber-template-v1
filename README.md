...
lifecycle contract with a Fiber starter on top.
...
- **Register every route on the group returned from `basePath()`**, never on the
  root app. In this template that group is ``app.Group(basePath())` in `newApp()``; a route added
  straight to the root app answers only when `BASE_PATH` is empty and 404s in
  the fleet.
